...
def random_lengths(seqs, adm, options=None,
                         duration=None, max_len=30, curr_time=106200):
    lengths = [0] * len(seqs)
    ...
    for i in range(len(seqs)):
        ...
        if pos >= 0: # pos >= 0 means there is some adm happened before max_time
            ...
            for turn in range(10):
                lengths[i] = numpy.random.randint(low, pos + 1) + 1 #randomly select a time point
                ...
                if (num_readm > 2) and (lengths[i] > 1):
                    break
    return lengths

def prepare_data(lengths, seqs, adm, options=None,
                 duration = None, max_len=30, curr_time = 106200):
...
def build_model(shared_params, options, use_noise = None):
    ...
    if 'drhid' in options['reg']:
        hid1 = lstm_layer.dropout_layer(hid1, use_noise, trng, 0.75)
    ...
    w = theano.shared(numpy.array([1.0, 3.0]))
    pr = pred / w[None, :]
    cost = -tensor.log(pr[tensor.arange(n_samples), y] + esp).mean()
