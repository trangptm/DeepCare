...
def init_top_params(options, params):
    ...
    params['V2'] = 1.0 * numpy.random.rand(options['dim_emb'], options['n_diag']).astype(config.floatX)

    params['c1'] = numpy.zeros((options['dim_emb'],)).astype(config.floatX)
    params['c2'] = numpy.zeros((options['n_diag'],)).astype(config.floatX)

    return params

def random_lengths(seqs, adm, max_len=30):
    lengths = [0] * len(seqs)
...
def prepare_data(lens, seqs, adm, duration=None, options=None, max_len=30):

    pm_len = [len(pm) for pm in adm['pm']]
    lengths = [min(len(seq) - 1, max_len) for seq in seqs]
    n_samples = 0
    for seq in seqs:
        if len(seq) > 1: n_samples += 1
    ...
    for i, seq in enumerate(seqs):
        ...
        y[:lengths[i], idx] = seq[1:lengths[i] + 1]
...
def prepare_train(seqs, adm, duration=None, options=None):
    lengths = random_lengths(seqs, adm)
    x, x_mask, time, med, y = prepare_data(lengths, seqs, adm)
    return x, x_mask, time, med, y
...
def build_model(shared_params, options, use_noise = None):
    ...
    proj = lstm_layer.lstm_layer(shared_params, options, emb_dia, emb_pm, x_mask, x_time, method)
    ...
    sorted_idx = pred.reshape([n_steps, n_samples, options['n_diag']]).argsort(axis=2)
    ...
    diag = next_diag + (tensor.arange(next_diag.shape[0]) * options['n_diag'])[:, None]

    diag_pred = pred[diag.flatten()].reshape([next_diag.shape[0], next_diag.shape[1]])
    diag_pred = (diag_pred * diag_mask).sum(axis=1)
    diag_pred = diag_pred / diag_mask.sum(axis=1)

    diag_pred = (diag_pred.reshape([n_steps, n_samples]) * x_mask[:, 0]).sum(axis=0)
    ...
    for i in range(n_steps):
        for j in range(n_samples):
            ...
            for t in range(n_pred):
                if pred[i, j, t] in adm['diag'][y[i, j]]:
                    acc[t] += 1
